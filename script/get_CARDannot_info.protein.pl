...
open OUT2, ">$out.filter.homolog.txt" or die ;
open OUT3, ">$out.filter.variant.txt" or die;
  ...
  chomp;
	...
	if (exists $mutations{$cut[4]}){
  	...
  	foreach my $k(@vars){
  		$k =~ s/\)//;
  		my ($type, $mut)=split (/\(/, $k);
  		if ($type eq "single resistance variant"){
...
#								print "multiple: $cut[4]...$count...$i...$start...$n...$alt...$alt_new...$len\n";
		  	...
		  	$count{$cut[0]}+=$index;
		  }elsif($type eq "nonsense mutation"){
		  	 my $alt = $2 if ($mut =~ /([A-Za-z]+)([0-9]+)([A-Za-z]+)/) ; 
		  	 my $alt_len = $cut[2] + $ali_len - 1;
		  	 if ($alt_len == $q_len){$index = 1; push @{$parameter{$cut[0]}}, $mut;}else{$index = 0 ;}
		  	 $count{$cut[0]}+=$index;
