...
sub parse_blast{
	...
	print "#Query_id\tQuery_length\tQuery_start\tQuery_end\tSubject_id\tSubject_length\tSubject_start\tSubject_end\t",
		"Identity\tPositive\tGap\tAlign_length\tScore\tE_value\tQuery_Align_Sequence\tSbjct_Align_Sequence\n" unless(defined $nohead);
	...
	while (<BLAST>) {
		...
		for (my $i=0;$cycle[$i];$i++) {
			...
			for (my $j=0;$cycle_inner[$j];$j++) {
				...
				#Gap信息不是每一组里都有,单独考虑
#				print "$query...$subject...$cycle_inner[$j]\n";
				$pointer->[$i][$j]{q_start}=$1 if($cycle_inner[$j]=~/Query\D+(\d+)(\D+)/); 
				$pointer->[$i][$j]{s_start}=$1 if($cycle_inner[$j]=~/Sbjct\D+(\d+)(\D+)/);
			  
			  my @arrs=split (/\n/, $cycle_inner[$j]);
			  for (my $k=0; $k<=$#arrs; $k++){
			  	if ($arrs[$k] =~ /Query\D+\d+(\D+)/){
			  		push @{$seqs{$query}{$subject}{Query}}, $1;
			  	}
			  	if ($arrs[$k] =~ /Sbjct\D+\d+(\D+)/){
			  		push @{$seqs{$query}{$subject}{Sbjct}}, $1;
			  	}
			  }
			  my $query_str = join ("", @{$seqs{$query}{$subject}{Query}});
			  $query_str =~ s/\s+//g;
			  $query_str =~ tr/a-z/A-z/;
			  my $subject_str = join ("", @{$seqs{$query}{$subject}{Sbjct}});
			  $subject_str =~ s/\s+//g;
			  $subject_str =~ tr/a-z/A-z/;
#			  print "$query...$subject...$str\n";
				...
				print "$query\t$query_len\t$pointer->[$i][$j]{q_start}\t$pointer->[$i][$j]{q_end}\t",
					...
					"$pointer->[$i][$j]{score}\t$pointer->[$i][$j]{e_value}\t$query_str\t$subject_str\n";
			}
		}
			...
			for (my $j=0;$cycle_inner[$j];$j++) {
			  ...
			  $subject_str =~ tr/a-z/A-z/;
				...
				print "$query\t$query_len\t$pointer->[0][$j]{q_start}\t$pointer->[0][$j]{q_end}\t",
					...
					"$pointer->[0][$j]{score}\t$pointer->[0][$j]{e_value}\t$query_str\t$subject_str\n";
			}
		}
	}
	$/="\n";
