...
sub nhx_str2arr
{
	...
	while (length($tree_str) > 0) {
		if ($tree_str =~ /^\(/) {
			...
			if($supp_info =~ /\d+/ && $nhx_info eq "") {
				$nhx_info = "\[&&NHX:B=$supp_info\]";
				$supp_info = "";
			}
			if($nhx_info =~ /\[(\d+)\]/) {
				$nhx_info = "\[&&NHX:B=$1\]";
...
## get MRCA node of two taxon nodes;
#sub mrca {
#	my ($self, $tp1, $tp2) = @_;
#
#	my (@par1, @par2);
#	while (defined $tp1->{parent}) {
#		push @par1, $tp1->{parent};
#		$tp1 = $tp1->{parent};
#	}
#	while (defined $tp2->{parent}) {
#		push @par2, $tp2->{parent};
#		$tp2 = $tp2->{parent};
#	}
#	print STDERR scalar(@par1), ", ", scalar(@par2), "\n";
#	for (my $i=$#par1; $i>=0; $i--) 
#	{
#		print STDERR "$par1[$i]->{id}\n";
#		for (my $j=$#par2; $j>=0; $j--) 
#		{
#			if ($par2[$j]->{id} eq $par1[$i]->{id}) {
#				return $par2[$j];
#			}
#		}
#	}
...
sub set_outgroup {
	...
	{
		if (($taxon =~ /^\d+$/ && $p->{id} eq $taxon) || $p->{name} eq $taxon) {
			$outgrp_node = $p->{parent};
			$new_root = $p->{parent};
		}
	}
	return if (!defined $outgrp_node);
