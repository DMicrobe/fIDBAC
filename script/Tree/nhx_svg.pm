...
sub plot_core{
	...
	foreach my $p (@$array) {
		...
		&& $p->{N}) {
			...
			my $italic="normal";
			if ($self->{taxon_name_style}) { $italic="italic"; }
			$svg->text('x',$p->{x}+$self->{half_box}+4,'y',$p->{y}+textHeight($self->{fsize})/2,'fill',$self->{c_exter},'-cdata',$outname,'font-size',$self->{fsize}, 'font-family',$self->{font}, 'font-style', $italic);
		}
	}
	...
	if( $self->root->{C} && @{$$self{_root}{C}} == 2 && $self->{show_root_branch}){
