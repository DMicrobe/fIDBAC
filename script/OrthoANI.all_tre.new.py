...
class MyThread(Thread):
    def __init__(self,query,sb,out,sem):
        super().__init__()
        self.q=query
        self.s=sb
        self.temp=out
    ...
    def run(self):
        cmd="perl {bin}/Average_Nucleotide_Identity/ANI/ANI.pl -qr {inf} -sb {sb} -od {temp}".format(bin=binname,inf=self.q,sb=self.s,temp=self.temp)
        os.system(cmd)
        time.sleep(1)
        self.sem.release()
...
def main(argv):
    ...
    make_dir(temp)
    threads=[]
    cmd="echo "+r'"'+infile+r'"'+"|sed -e 's/;/\\n/g' >"+temp+"/genome.lst"
    listfile=temp+"/genome.lst"
    os.system(cmd)
    rmd=[]
    genomelist=[]
    sample=['input']
    ...
    if os.path.exists(listfile):
        with open (listfile,'r') as handle:
            ...
            for line in handle:
                ...
                sem.acquire()
                thread = MyThread(infasta,line,tempdir,sem)
                thread.start()
    ...
    for i in range(len(genomelist)):
        ...
        for j in range(i+1,len(genomelist)):
            ...
            sem.acquire()
            thread = MyThread(genomelist[i],genomelist[j],tempdir,sem)
            thread.start()
...
if __name__ == '__main__':
    if len(sys.argv) < 1 :
        ...
        sys.exit()
    binname, filename = os.path.split(os.path.abspath(__file__))
